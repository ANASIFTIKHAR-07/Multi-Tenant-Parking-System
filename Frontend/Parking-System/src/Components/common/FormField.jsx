...
export function TextInput({ label, required, error, hint, className = '', ...props }) {
  return (
    <FormField label={label} required={required} error={error} hint={hint}>
      <input className={`${inputBase} ${error ? 'border-red-400 focus:ring-red-200/50' : 'border-slate-200 dark:border-slate-700/80'} ${className}`} {...props} />
    </FormField>
  );
}
...
export function SelectInput({ label, required, error, hint, children, className = '', ...props }) {
  return (
    <FormField label={label} required={required} error={error} hint={hint}>
      <select className={`${inputBase} ${error ? 'border-red-400 focus:ring-red-200/50' : 'border-slate-200 dark:border-slate-700/80'} appearance-none ${className}`} {...props}>
        {children}
      </select>
    </FormField>
  );
...
export function TextareaInput({ label, required, error, hint, className = '', ...props }) {
  return (
    <FormField label={label} required={required} error={error} hint={hint}>
      <textarea rows={3} className={`${inputBase} ${error ? 'border-red-400 focus:ring-red-200/50' : 'border-slate-200 dark:border-slate-700/80'} resize-none ${className}`} {...props} />
    </FormField>
  );
}
