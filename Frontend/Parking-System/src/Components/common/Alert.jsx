...
const CFG = {
  error:   { wrap: 'bg-red-50 border-red-200',     icon: 'text-red-500',     text: 'text-red-800'     },
  success: { wrap: 'bg-emerald-50 border-emerald-200', icon: 'text-emerald-500', text: 'text-emerald-800' },
  warning: { wrap: 'bg-amber-50 border-amber-200',  icon: 'text-amber-500',   text: 'text-amber-800'   },
  info:    { wrap: 'bg-blue-50 border-blue-200',    icon: 'text-blue-500',    text: 'text-blue-800'    },
};
...
export default function Alert({ type = 'error', message, onDismiss }) {
  ...
  return (
    <div className={`flex items-start gap-2.5 px-3.5 py-3 rounded-xl border text-[13px] ${c.wrap}`}>
      <svg className={`w-4 h-4 mt-0.5 flex-shrink-0 ${c.icon}`} fill="currentColor" viewBox="0 0 20 20">
        {ICONS[type]}
      </svg>
      <p className={`flex-1 font-medium leading-snug ${c.text}`}>{message}</p>
      {onDismiss && (
        <button onClick={onDismiss} className={`${c.icon} hover:opacity-70 transition-opacity flex-shrink-0 mt-0.5`}>
