...
export default function Employees() {
  ...
  const openCreate = () => { setForm(EMPTY_EMP); setEditing(null); setModal('form'); };
  const openEdit = (emp) => {
  ...
  const columns = [
    { key: 'full_name', label: 'Employee', render: r => (
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {r.id_card_number && <p className="text-xs text-slate-400 font-mono">{r.id_card_number}</p>}
        </div>
      </div>
    )},
    { key: 'tenant', label: 'Company', render: r => (
      <div>
        <p className="text-sm text-slate-700">{r.tenant_id?.company_name || '—'}</p>
    ...
    { key: 'badge', label: 'Badge', render: r => r.active_badge ? (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-xs font-semibold ring-1 ring-emerald-100">
        #{r.active_badge.badge_number}
      </span>
    ) : <span className="text-slate-300 text-xs">None</span> },
    { key: 'vehicles', label: 'Vehicles', render: r => (
      <span className="text-xs text-slate-500">{r.vehicles?.length || 0} registered</span>
    )},
    { key: 'actions', label: '', render: r => (
      <div className="flex items-center gap-1.5 justify-end">
        <button onClick={() => openDetail(r)} className="px-2.5 py-1.5 text-xs font-semibold rounded-lg bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors">View</button>
        <button onClick={() => openAddVehicle(r)} className="px-2.5 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors">+ Vehicle</button>
      ...
      <Modal isOpen={modal === 'vehicle'} onClose={() => setModal(null)} title={`Add Vehicle — ${editing?.full_name}`}>
        <form onSubmit={handleAddVehicle} className="space-y-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={vehicleForm.is_primary} onChange={e => setVehicleForm(f => ({ ...f, is_primary: e.target.checked }))} className="w-4 h-4 rounded text-blue-600" />
            <span className="text-sm font-medium text-slate-700">Set as primary vehicle</span>
          </label>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setModal(null)} className="px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">Cancel</button>
            <button type="submit" disabled={submitting} className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-sm font-semibold rounded-xl shadow-sm transition-colors">
        ...
        {detailEmployee && (
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><p className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1">Company</p><p className="font-medium text-slate-800">{detailEmployee.tenant_id?.company_name}</p></div>
              <div><p className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1">Job Title</p><p className="font-medium text-slate-800">{detailEmployee.job_title || '—'}</p></div>
              <div><p className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1">ID Card</p><p className="font-mono text-slate-800">{detailEmployee.id_card_number || '—'}</p></div>
              <div><p className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1">Status</p><StatusBadge status={detailEmployee.status} /></div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Registered Vehicles</p>
                <button onClick={() => { setModal(null); setTimeout(() => openAddVehicle(detailEmployee), 100); }} className="text-xs font-semibold text-blue-600 hover:text-blue-700">+ Add Vehicle</button>
              </div>
              {detailEmployee.vehicles?.length === 0 ? (
                ...
                <div className="space-y-2">
                  {detailEmployee.vehicles?.map((v) => (
                    <div key={v.car_plate_number} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold font-mono text-slate-800">{v.car_plate_number}</p>
                          <p className="text-xs text-slate-400">{v.sticker_number ? `Sticker: ${v.sticker_number}` : ''}{v.car_tag ? ` · Tag: ${v.car_tag}` : ''}</p>
                        </div>
                        {v.is_primary && <span className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold ring-1 ring-blue-100">Primary</span>}
                      </div>
                      <button onClick={() => handleRemoveVehicle(detailEmployee._id, v.car_plate_number)} className="text-xs text-red-500 hover:text-red-700 font-medium">Remove</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            {detailEmployee.active_badge && (
              <div>
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-2">Active Badge</p>
                <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-100 flex items-center gap-3">
                  <svg className="w-4 h-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0" /></svg>
                  <span className="text-sm font-semibold text-emerald-800">Badge #{detailEmployee.active_badge.badge_number}</span>
                  {detailEmployee.active_badge.sr_number && <span className="text-xs text-emerald-600">SR: {detailEmployee.active_badge.sr_number}</span>}
                </div>
              </div>
            )}
            {detailEmployee.parking_records?.length > 0 && (
              <div>
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-2">Active Parking</p>
                {detailEmployee.parking_records.map(p => (
                  <div key={p._id} className="p-3 rounded-xl bg-blue-50 border border-blue-100 flex items-center gap-3">
                    <StatusBadge status={p.parking_type} />
                    <span className="text-sm font-mono font-semibold text-blue-800">{p.car_plate_number}</span>
                    {p.assigned_slot?.slot_code && <span className="text-xs text-blue-600">Slot: {p.assigned_slot.slot_code}</span>}
                  </div>
                ))}
              </div>
            )}
