...
export default function Units() {
  ...
  const columns = [
    { key: 'floor', label: 'Floor', render: r => <span className="font-semibold text-slate-800">{r.floor}</span> },
    { key: 'unit_number', label: 'Unit No.', render: r => <span className="font-mono text-slate-700">{r.unit_number}</span> },
    ...
    { key: 'max_card_limit', label: 'Card Limit', render: r => (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold ring-1 ring-blue-100">{r.max_card_limit}</span>
    )},
    { key: 'owner', label: 'Owner', render: r => (
      <div>
        <p className="text-sm font-medium text-slate-700">{r.owner?.name}</p>
        {r.owner?.qb_code && <p className="text-xs text-slate-400">{r.owner.qb_code}</p>}
      </div>
    )},
    { key: 'actions', label: '', render: r => (
      <div className="flex items-center gap-2 justify-end">
        <button onClick={() => openEdit(r)} className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors">Edit</button>
        <button onClick={() => handleDelete(r)} className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition-colors">Delete</button>
      </div>
    )},
  ];
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-[140px]">
          ...
          <input value={filters.floor} onChange={e => { setFilters(f => ({ ...f, floor: e.target.value })); setPage(1); }}
            placeholder="e.g. 1, 2, B1" className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all" />
        </div>
        <div className="flex-1 min-w-[140px]">
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1.5">Zone</label>
          <input value={filters.zone} onChange={e => { setFilters(f => ({ ...f, zone: e.target.value })); setPage(1); }}
            placeholder="Zone A, B..." className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all" />
        </div>
        <button onClick={() => { setFilters({ floor: '', zone: '' }); setPage(1); }} className="px-4 py-2 text-sm font-medium text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-xl transition-colors">
          Clear
        </button>
      </div>
